...
from Data_Augmentation.BackgroundFileInterface  import BackgroundFileLoader
from Data_Augmentation.SampleImgInterface import SampImgModifier

DEFAULT_PARAMS={
'BackgroundFilePath':'/home/bionicdl-razer/Downloads/val2017',
...
def createSamples(config_file):
    ...
    print("Out put image path is %s"%outputfolder)
    bkgFileLoader=BackgroundFileLoader()
    bkgFileLoader.loadbkgFiles(backgroundFilePath)
    for sampleImgPath in glob.glob(os.path.join(samplePath,'*.png')):
    ...
    print("Generated %s images with labels"%(len(glob.glob(os.path.join(samplePath,'*.png')))*outputPerSample))
    return outputfolder
